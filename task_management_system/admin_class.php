...
Class Action {
    ...
    function save_subactivity(){
        ...
        if(empty($id)){
            $save = $this->db->query("INSERT INTO sub_activity set $data");
        }else{
            $save = $this->db->query("UPDATE sub_activity set $data where id = $id");
        }
        if($save){
            return 1;
        }
    ...
    function delete_subactivity(){
        extract($_POST);
        $delete = $this->db->query("DELETE FROM sub_activity WHERE id = $id");
        if($delete){
            return 1;
        } else {
            return 0; // Return 0 or false to indicate failure
        }
    }
